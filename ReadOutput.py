...
Readlog = get_logger(childName='Read', log_dir='Runninglog')
...
def Read_Output(Output輸出檔):
    try:
        ...
        if '匯出檔' in 工作表名稱:
            ...
            Output = pd.concat(
                [Output_DIP[['母工單單號', '工號', '名稱規格', '工令量', 'SOURCE']],
                 Output_SMT[['母工單單號', '工號', '名稱規格', '工令量', 'SOURCE']]],
                ignore_index=True)
        ...
        # 使用logging.error來記錄錯誤信息，包括異常的詳細信息
        # Readlog.error(f'Output讀取出現錯誤: {str(e)}')
        # 使用logging.exception記錄異常的詳細堆棧信息
        Readlog.exception('Output讀取出現錯誤:')
